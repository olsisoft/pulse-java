...
```bash
# Main + FeedAndRead use the pulse-java SDK on $CP. (NativeFeeder is built
# separately in "Run on the cluster" — it needs the engine SDK, not $CP.)
javac -cp "$CP" -d /tmp/ex \
    examples/cobol-mainframe-modernization/Main.java \
    examples/cobol-mainframe-modernization/FeedAndRead.java
export PULSE_URL=http://localhost:9090 PULSE_USER=alice PULSE_PASSWORD=secret

java -cp "$CP:/tmp/ex" Main                            # upload WASM + deploy the pipeline
java -cp "$CP:/tmp/ex" FeedAndRead feed                # publish sample COBOL records
java -cp "$CP:/tmp/ex" FeedAndRead read cobol-parsed   # see the WASM-parsed JSON
java -cp "$CP:/tmp/ex" FeedAndRead read modernized-events   # see the wasm→rule→llm output
```

`FeedAndRead` talks to Pulse's HTTP event API (`/api/pulse/events`), so it needs
no Kafka client — it works against a standalone Pulse.
